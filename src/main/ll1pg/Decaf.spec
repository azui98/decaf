...
Type            :   AtomType TypeSuffix
                    {
                        $$ = $1;
						//assert($2.intVal == $$.);
                        for (int i = 0; i < $2.intVal; i++) {
                            //$$.type = new TArray($$.type, $1.type.pos);
							var temp = $2.thunkList.get(i);
							if (temp.isTemporary())                         // appending '[' ']'
								$$.type = new TArray($$.type, $1.type.pos);
							else                                            // appending '(' TypeList ')'
								$$.type = new TFunc($$.type, temp.typeList, $1.type.pos);
                        }
                    }
                ;
				...
				|   '(' TypeList ')' TypeSuffix
					{
						$$ = $4;
						$$.intVal++;
						$$.thunkList.add(0, svTypes($2.typeList));
					}
				|   /* empty */
                    {
                        $$ = new SemValue();
