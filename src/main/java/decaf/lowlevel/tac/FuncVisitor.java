...
public class FuncVisitor {
	/**
	 * For a member method, 'this' pointer will be its last arg.
	 * Same for lambda expressions in any member method.
	 */
	public boolean hasThisArg;
	
	/**
	 * For a lambda expression, after a traverse, the Temp arrangement
	 * may be illegal for the simulater. (Captured args may not be the first Temps)
	 * Use this 'map' to find the correct Temp arrangement and replace it
	 */
	public Map<Temp, Temp> map = new HashMap<Temp, Temp>();
	
	//Consumer<FuncVisitor> s = v -> {};
	
	public boolean isLambda = false;
	private Map<Temp, Temp> captured = new HashMap<Temp, Temp>();
	private List<Temp> capTemps = new ArrayList<Temp>();
	
	public final Map<Temp, Temp> getCaptured() {
		return captured;
	}
	
	public Temp capture(Temp origin) { // origin is not difined in this FuncVisitor
		if (captured.containsKey(origin))
			return captured.get(origin);
		var newAssigned = freshTemp();
		captured.put(origin, newAssigned);
		capTemps.add(origin);
		return newAssigned;
	}
    ...
    public Temp visitBinary(TacInstr.Binary.Op op, Temp lhs, Temp rhs) {
        ...
        if (op != TacInstr.Binary.Op.DIV && op != TacInstr.Binary.Op.MOD)
        	...
        	var ok = freshLabel();
        	Temp cond = visitBinary(TacInstr.Binary.Op.EQU, rhs, visitLoad(0));
        	visitBranch(TacInstr.CondBranch.Op.BEQZ, cond, ok);
        	visitPrint("Decaf runtime error: Division by zero error\n");
        	visitIntrinsicCall(Intrinsic.HALT);
        	visitLabel(ok);
        	func.add(new TacInstr.Binary(op, temp, lhs, rhs));
        }
        return temp;
    ...
    public Temp visitStaticCall(String clazz, String method, List<Temp> args, boolean needReturn) {
        ...
        //System.out.println(clazz + " - " + method + " . " +entry);
    ...
    /**
     * Calling by an explicit lambda expression
     * For example
     * (fun(int x) => x)(1)
     */
    public Temp visitLambdaCall(String id, List<Temp> args, boolean needReturn) {
    	var entry = ctx.getLambdaLabel(id);
    	args.forEach(arg -> func.add(new TacInstr.Parm(arg)));
    	Temp temp = null;
    	if (needReturn) {
            temp = freshTemp();
            func.add(new TacInstr.DirectCall(temp, entry));
     ...
     * For example
     * var f = fun(int x) { return x; };
     * int u = f(1);
     */
    public Temp visitStoredFunc(Temp FuncAddr, boolean needReturn) {
    	Temp temp = null;
        if (needReturn) {
            temp = freshTemp();
    ...
    /**
     * Calling a NON-STATIC member method by its address
     */
    public Temp visitStoredMemberMethod(Temp object, Temp VTableAddr, List<Temp> args, boolean needReturn) {
    	Temp temp = null;
        var entry = visitLoadFrom(VTableAddr, 8);
        func.add(new TacInstr.Parm(object));
    	args.forEach(arg -> func.add(new TacInstr.Parm(arg)));
        if (needReturn) {
            temp = freshTemp();
    ...
    List<Temp> tempRef = new ArrayList<Temp>();
    /**
     * Create a fresh temp.
     *
     * @return temp
    ...
    public Temp getArgTemp(int index) {
    	//System.out.println(argsTemps.length + " wtf  " + index);
    ...
    FuncVisitor(FuncLabel entry, int numArgs, ProgramWriter.Context ctx, boolean passThis) {
        this.ctx = ctx;
        func = new TacFunc(entry, numArgs);
        this.hasThisArg = passThis;
        visitLabel(entry);
    ...
    FuncVisitor(FuncLabel entry, int numArgs, ProgramWriter.Context ctx, boolean isLambda, boolean passThis) {
    	this.ctx = ctx;
        this.isLambda = isLambda;
        if (passThis) numArgs++; // for 'this'
        func = new TacFunc(entry, numArgs);
        this.numArgs = numArgs;
        this.hasThisArg = passThis;
        visitLabel(entry);
    ...
    public int argCnt() { return numArgs; }
    
    public void appendCaptured() {
    	if (capTemps == null) return;
    	var old = argsTemps;
    	argsTemps = new Temp[old.length + capTemps.size()];
    	for (int i = 0; i < old.length; i++) {
    		argsTemps[i] = old[i];
    	}
    	for (int i = 0; i < capTemps.size(); i++) {
    		argsTemps[old.length + i] = captured.get(capTemps.get(i));
    	}
    	
    	/**
    	 * The following two loops will rearrange the order of temps
    	 * so that captured args will put just behind the original args.
    	 * Order : [Original args] (this) [Captured args] [Local temps] 
    	 * Use map.get(oldTemp) to get the newly arranged temp
    	 */
    	int cnt = old.length;
    	for (int i = 0; i < nextTempId; i++) {
    		if (i < old.length) map.put(old[i], old[i]);
    		else if (captured.containsValue(tempRef.get(i)))
    			map.put(tempRef.get(i), tempRef.get(cnt++));
    	}
    	for (int i = 0; i < nextTempId; i++) {
    		if (i >= old.length && !captured.containsValue(tempRef.get(i)))
    			map.put(tempRef.get(i), tempRef.get(cnt++));
    	}
    ...
    private int nextTempId = 0, numArgs = 0;
