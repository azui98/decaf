...
public class ProgramWriter {
    ...
    public FuncVisitor visitMainMethod() {
        ...
        return new FuncVisitor(entry, 0, ctx, false);
    }
     ...
     */
    public FuncVisitor visitFunc(String className, String funcName, int numArgs, boolean passThis) {
        var entry = ctx.getFuncLabel(className, funcName);
        return new FuncVisitor(entry, numArgs, ctx, passThis);
    }
    
    /**
     * Generate TAC for a lambda expression.
     * 
     * @param id id for a lambda expression
     * @param numArgs number of arguments
     */
    public FuncVisitor visitFunc(String id, int numArgs, boolean passThis) {
    	var entry = ctx.getLambdaLabel(id);
    	return new FuncVisitor(entry, numArgs, ctx, passThis);
    }
    
    public FuncVisitor visitFunc(String id, int numArgs, boolean isLambda, boolean passThis) {
    	var entry = ctx.getLambdaLabel(id);
    	return new FuncVisitor(entry, numArgs, ctx, isLambda, passThis);
    }
    
    /**
     * define a lambda expr
     */
    public void putLambdaLabel(String id) {
    	ctx.putLambdaLabel(id);
    }
    ...
    private void createConstructorFor(String clazz) {
        ...
        var mv = new FuncVisitor(entry, 0, ctx, false);
    ...
    public void buildVTableForLambda(String id) {
    	var vtbl = new VTable(id, Optional.empty());
    	var label = ctx.getLambdaLabel(id);
    	vtbl.memberMethods.add(label);
    	ctx.putVTable(vtbl);
    	ctx.putOffsets(vtbl);
    }
    

	public void buildVTableForMethod(String id) {
		if (ctx.hasVTable(id)) return;
		var vtbl = new VTable(id, Optional.empty());
		var label = ctx.getLambdaLabel(id);
		// In fact, it will return a method, because id = class.method
		vtbl.memberMethods.add(label);
		ctx.putVTable(vtbl);
		ctx.putOffsets(vtbl);
	}
    ...
    class Context {
        ...
        void putLambdaLabel(String id) {
        	labels.put(id, new FuncLabel("", id));
        }
        ...
        FuncLabel getLambdaLabel(String id) {
